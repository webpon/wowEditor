...
export const outlineRender = (contentElement: HTMLElement, targetElement: Element, vditor?: IVditor) => {
    ...
    headingsElement.forEach((item, index) => {
        if (item.nextElementSibling && item.nextElementSibling.tagName === "UL") {
            ...
            let iconHTML = "<svg class='vditor-outline__action vditor-outline__action--close'><use xlink:href='#vditor-icon-down'></use></svg>";
            if (!document.getElementById("vditorIconScript")) {
                iconHTML = '<svg class="vditor-outline__action vditor-outline__action--close" viewBox="0 0 32 32"><path d="M3.76 6.12l12.24 12.213 12.24-12.213 3.76 3.76-16 16-16-16 3.76-3.76z"></path></svg>';
            }
    ...
    targetElement.firstElementChild.addEventListener("click", (event: Event) => {
        ...
        while (target && !target.isEqualNode(targetElement)) {
            if (target.classList.contains("vditor-outline__action")) {
                ...
                let wraper = null
                if(document.querySelector('.vditor-wysiwyg') && (document.querySelector('.vditor-wysiwyg') as HTMLElement).style.display === 'block') {
                    wraper = document.querySelector('.vditor-wysiwyg')
                } else if(document.getElementsByClassName('vditor-ir')[0] && (document.getElementsByClassName('vditor-ir')[0] as HTMLElement).style.display === 'block') {
                    wraper = document.getElementsByClassName('vditor-ir')[0]
                }
                else {
                    return
                }
                ...
                if (vditor) {
                    ...
                    if (vditor.options.height === "auto") {
                        ...
                        wraper.scrollTo({
                            top: windowScrollY - 24 - document.querySelector('.vditor-toolbar').clientHeight - 16,
                            behavior: 'smooth'
                        })
                        const row = target.querySelector('.vditor-outline__action')
                        if (row && row.classList.contains("vditor-outline__action--close")) {
                            row.classList.remove("vditor-outline__action--close");
                            row.parentElement.nextElementSibling.setAttribute("style", "display:block");
                        } 
                    } else {
                        if (vditor.element.offsetTop < window.scrollY) {
                            idElement.scrollTo(window.scrollX, vditor.element.offsetTop);
                        }
                ...
                } else {idElement.scrollTo(window.scrollX, idElement.offsetTop);
                }
                break;
            }
