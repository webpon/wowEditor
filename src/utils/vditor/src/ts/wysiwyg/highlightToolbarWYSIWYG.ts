...
const setPopoverPosition = (vditor: IVditor, element: HTMLElement) => {
    ...
    vditor.wysiwyg.popover.style.right = (document.body.clientWidth - document.getElementsByClassName('vditor-reset')[0].getBoundingClientRect().right) + 30 + 'px';
    vditor.wysiwyg.popover.style.display = "block";
    vditor.wysiwyg.popover.style.top =
        Math.max(5, targetElement.offsetTop + targetElement.clientHeight) + "px";
...
const genClose = (element: HTMLElement, vditor: IVditor) => {
    ...
    close.setAttribute("aria-label", window.VditorI18n.remove + "<" + updateHotkeyTip("⌘D") + ">");
    close.innerHTML =
