...
export const processKeydown = (vditor: IVditor, event: KeyboardEvent) => {
    ...
    if (headingElement) {
        ...
        // 光标做处理
        if (event.key === "Enter" && range.startOffset === 0) {
            setTimeout(() => {
                const newElement = document.getElementById(headingElement.id)
                newElement.outerHTML = `<p data-block="0"></p>`;
            })
        }
        if (event.key === "Backspace" && !isCtrl(event) && !event.shiftKey && !event.altKey
            && (headingElement.textContent.length === 0 || headingElement.textContent === ' ')) {
            // 删除后变为空
